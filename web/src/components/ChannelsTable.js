...
const ChannelsTable = () => {
                    ...
                    <div>
                        <Space spacing={1}>
                            ...
                            <Tooltip content={'剩余额度，点击更新'}>
                                <Tag color='white' type='ghost' size='large' onClick={() => {updateChannelBalance(record)}}>{renderBalance(record.type,record.balance)}</Tag>
                            </Tooltip>
                        </Space>
                    </div>
                );
    ...
    const updateChannelBalance = async (record) => {
        ...
        if (success) {
            ...
            channels[channels.findIndex((channel) => channel.id === record.id)] = record;
            setChannels([...channels])
